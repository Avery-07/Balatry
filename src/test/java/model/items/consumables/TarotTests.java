...
public final class TarotTests {
    ...
    /** Targeted tarots refuse to fire into nothing, so a mis-click can never waste the card. */
    private static void targetRequirements() {
        Run run = new Run(5L);
        run.addConsumable(Tarots.STRENGTH.make());
        boolean refused = false;
        try { run.useConsumable(0, List.of()); } catch (IllegalStateException e) { refused = true; }
        check("Strength with no selection is refused", refused);
        check("and the card is not consumed", run.getConsumables().size() == 1);

        // Death demands exactly two; one is not enough.
        Run run2 = new Run(5L);
        run2.addConsumable(Tarots.DEATH.make());
        boolean refusedOne = false;
        try { run2.useConsumable(0, List.of(card(Rank.TWO, Suit.SPADES))); }
        catch (IllegalStateException e) { refusedOne = true; }
        check("Death with one selection is refused", refusedOne);

        // An untargeted tarot is unaffected: it still fires with nothing selected.
        Run run3 = new Run(5L);
        int before = run3.getMoney();
        run3.addMoney(10);
        run3.addConsumable(Tarots.THE_HERMIT.make());
        run3.useConsumable(0, List.of());
        check("Hermit still fires with no selection", run3.getMoney() == before + 20 && run3.getConsumables().isEmpty());

        // The data the client's greyed-out button reads.
        check("Strength declares 1 target", Tarots.STRENGTH.spec().getMinTargets() == 1);
        check("Death declares 2", Tarots.DEATH.spec().getMinTargets() == 2);
        check("The World declares 3", Tarots.THE_WORLD.spec().getMinTargets() == 3);
        check("Hermit declares none", Tarots.THE_HERMIT.spec().getMinTargets() == 0);
    }
