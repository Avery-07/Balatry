...
public final class JokerCard extends MarketCard {
    ...
    public int playedRetriggers(Run run, model.items.DeckCard card) { return spec.getPlayedRetrigger().extra(run, this, card); }

    /** Extra passes this joker grants {@code card} when it is held in hand. */
    public int heldRetriggers(Run run, model.items.DeckCard card)   { return spec.getHeldRetrigger().extra(run, this, card); }
