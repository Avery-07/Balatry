...
final class Overlays {
    ...
    void options(Ui ui, double now) {
        ...
        if (ui.showSettings)   { settings(ui); return; }
        ...
        double pw = 420, ph = 372, px = (Ui.W - pw) / 2, py = (Ui.H - ph) / 2;
        ...
        if (!ui.confirmSurrender) {
            ...
            ui.button(bx, py + 208, bw, 48, "Settings",   BLUE,   INK,  () -> ui.showSettings = true, true);
            ui.button(bx, py + 266, bw, 48, "Surrender",  RED,    INK,  () -> ui.confirmSurrender = true, true);
        } else {
    ...
    /** The Settings sub-panel, opened from Options: the CRT/VHS toggle (and room for more), plus Back. */
    private void settings(Ui ui) {
        Renderer r = ui.r;
        ui.buttons.clear();
        ui.tips.clear();
        r.gc().setFill(Color.web("#04060a", 0.7)); r.gc().fillRect(0, 0, Ui.W, Ui.H);

        double pw = 420, ph = 260, px = (Ui.W - pw) / 2, py = (Ui.H - ph) / 2;
        r.panel(px, py, pw, ph, Color.web("#161719"), ORANGE, 16, 3);
        r.textCenterBold("SETTINGS", px + pw / 2, py + 40, 26, ORANGE);

        double bw = pw - 80, bx = px + 40;
        boolean on = r.crtEnabled;
        r.textCenter("CRT / VHS effect", px + pw / 2, py + 80, 12, DIM);
        ui.button(bx, py + 98, bw, 48, on ? "CRT: On" : "CRT: Off",
                on ? GREEN : Color.web("#3a3d44"), on ? DARK : INK, () -> r.crtEnabled = !r.crtEnabled, true);
        ui.button(bx, py + ph - 62, bw, 48, "Back", ORANGE, DARK, () -> ui.showSettings = false, true);
    }
