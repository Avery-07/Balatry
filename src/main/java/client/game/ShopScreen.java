...
final class ShopScreen implements Screen {
    ...
    private static final double TILE_W = 108, TILE_H = 130, GAP = 10;
    ...
    public void render(Ui ui, double x, double y, double w, double h) {
        ...
        shelf(ui, ui.shopSlotRow, shop.slots().size(), ix + iw / 2, cy, iw,
                i -> {
        ...
        double half = iw / 2 - 24;
        shelf(ui, ui.shopVoucherRow, shop.vouchers().size(), ix + iw / 4, cy, half,
                i -> {
        ...
        shelf(ui, ui.shopPackRow, shop.packs().size(), ix + iw * 3 / 4, cy, half,
                i -> {
     ...
     */
    private void shelf(Ui ui, TileRow row, int slotCount, double centerX, double y, double maxWidth, IntFunction<Tile> tileAt) {
        // One centered, count-scaled line for the whole shelf (Layout.slots): a sold slot keeps its computed
        // position as a static hole, live tiles ride the retained row so a held one lifts and glides home.
        double[] all = Layout.slots(slotCount, centerX, TILE_W, GAP, maxWidth);
        ...
        for (int i = 0; i < slotCount; i++) {
            ...
            if (t == null) {   // a spent slot: a hole in the shelf, not a tile in the row
                ui.r.panel(cx - TILE_W / 2, y + 16, TILE_W, TILE_H, javafx.scene.paint.Color.web("#1a1b1f"), EDGE, 8, 2);
